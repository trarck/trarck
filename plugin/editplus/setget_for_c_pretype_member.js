...
//var input="ISOCoordinateLayer:int mapWidth,int mapHeight";
//col=0;
//
//prePad=pad(col," ");
//
...
function synthesizePropertyContent(propName,type,cls) {
	...
	var caseAdjusted=ucfirst(propName);
	var typePropName=getNameWithPreOfType(propName,type);
    var lVarName = 'm_' + typePropName;
	...
	var getter=inline+type+" "+preCls+"get"+caseAdjusted+"()\n{\n"
	          + tab+"return "+lVarName+";\n"
	          +"}\n";
...
function getNameWithPreOfType(name,type) {
    ...
    if(type.indexOf("*")>-1){
        ...
        if(/(^|\s)(int|long|char|float|double|bool|string|std::string)($|\s|&)/.test(type)){
