	...
	for(var i in versions){
		(function(){
			...
			version_tasks.enqueue(function(done){
                ...
                var tp = new Async.WorkPool();
                //main manifest
                tp.enqueue(function(done){
                    generateContents(output_dir,base_path,manifest,COMBINE_MODE,mainManifest,function(mainBuffer,mainScripts){
                        buffer=mainBuffer;
                        scripts=mainScripts;
                        done();
                    });
                ...
                tp.enqueue(function(done){
                    var subManifest=manifest[version];
                    if(version && subManifest){
                        generateContents(output_dir,base_path,manifest,COMBINE_MODE,mainManifest,ts.task(function(subBuffer,subScripts){
                            //merge to main manifest
                            for(var k in subBuffer){
                                buffer[k]=subBuffer[k];
                            }
                            for(var i=0,l= subScripts.length;i<l;i++){
                                if(utils.indexOf(scripts,subScripts[i])==-1){
                                    scripts[i]=subScripts[k];
                                }
                            }
                            done();
                        }));
...
function generateContents(output_dir,base_path,manifest,combineMode,mainManifest,callback){
    var buffer = {};
    var scripts = [];
    ...
    if(combineMode){
        scripts = js_include_list(manifest, combineMode, mainManifest);
        callback(buffer,scripts);
    }
    else{
        Combiner.getAllRequires(manifest, base_path,function(requires){
