...
var gLogRoot = './Logs';
var gTestServer = undefined;
var gTestServerPort = 6868;
...
{
	utils.info("server : copyConfiguration base_path=%s", base_path);
    ...
    if (data) {
      ...
      output_root = utils.replacePathToSlash(output_root);
      ...
      // Platform specific appIDs
      var appId = aobj.appId;
      if (aobj.appId)
      {
        aobj.appId = appId['android'] || appId;
        iobj.appId = appId['ios'] || appId;
      }
      ...
      utils.info("server : No configuration.json found." + e.toString());
...
exports.start = function(server_options){
  ...
  if ( server_options.testServer )
    gTestServer = server_options.testServer;

  if ( server_options.testServerPort )
    gTestServerPort = server_options.testServerPort;
...
*/
	...
	var getManifest = function(base_path){
		if(base_path){
			if (manifests[base_path] && manifests[base_path].length > 0) {
//				utils.info("server : getManifest  cache " + base_path + " '" + manifests[base_path]  +"' " + JSON.stringify(manifests[base_path]));
				return manifests[base_path];
			}
			var p = path.join(base_path, 'manifest.json');
			if(File.exists(p)){
				var value = JSON.parse(File.readFileSyncSafe(p));
				manifests[base_path] = [value, p];
  ...
  function sendTestServer(request, response) {
    if (gTestServer) {
      sendServerResponse(response, 200, 'http://' + gTestServer + ':' + gTestServerPort, 'text/plain');
    } else {
      sendServerResponse(response, 200, '', 'text/plain');
    }
  }
	...
	var server = http.createServer(function(request, response)
	{
		...
		if (utils.enableDebug()) utils.debug("http start : request.url='" + request.url + "'");
		...
		if(urlMatch) {
			switch(urlMatch[0]) {
				...
				case '/echo_post':
					...
					return;
        case '/testserver':
          sendTestServer(request, response);
          return;
			}
		}
	    ...
	    if(uri == '/') {
	    	entryList(uri, response);
			return;
	    }
		var base = Manifest.getBasePath(uri);
		var retArray = getManifest(base);

		var manifest = null;
		var manifestPath  = null; 
		if (retArray) {
			manifest      = retArray.shift();
			manifestPath  = retArray.shift();
		}
...
// 					}
				...
				manifestQueue.enqueue(function (done) {
					Assets.process_assets(manifestPath, path.join(process.cwd(), base, 'build'), base, server_options, function(){
						// for Flash
						if (utils.isFlash(request) && !utils.isCurl(request)) {
								utils.debug("webgame.ngmanifest for flash");
							...
							Combiner.combine(manifest, path.join(process.cwd(), base, 'build'), base, server_options, function(err){
								Manifest.create_index(manifest, path.join(process.cwd(), base, 'build'), base, server_options, function(){
									Manifest.create_manifest(manifest, path.join(process.cwd(), base, 'build'), "", base, server_options, function(){
										Manifest.genSecondaryManifests(path.join(process.cwd(), base, 'build'), base, server_options, function(){
											returnFile(uri, response, done);
		...
		{
			utils.info('Exception:' + e.toString());
			for(var i in e)
				utils.info(i + ': ' + e[i]);
	...
	function returnFile(uri, response, done){
		...
		path.exists(filename, function(exists)
		{
			...
			if(!exists)
			{
				...
				response.end();
				utils.warn('server : Not found uri  ' + uri + '\n');
				utils.warn('server : Not found file ' + filename + '\n');
				if(done) {done();}
			}
			else
			{
...
//					utils.info(" Returning file " + uri + "\n");
	...
	function entryList(uri, response) {
		console.log("entryList");
		_searchManifestFiles(process.cwd(), function(files) {
			console.log("ListReturned: " + files);
			var contents = '';
			var currentDir = process.cwd();
			utils.info(JSON.stringify(files));
			for( var i = 0; i < files.length ; i++) {
				var file = files[i];
				utils.debug("serchFiles : %s ", files[i]);
				file = file.replace(currentDir, '').replace('manifest.json','');
	...
	function _searchManifestFiles(directory, callback) {
		var files = [];
		fs.readdir(directory, function(err, filenames) {
			if (err) {
				...
				var waitingForResponses = filenames.length;
				...
				var checkFile = function(directory, filename) {
					...
					fs.lstat(path, function(err, stat) {
						...
						//	throw err;
							...
							_searchManifestFiles(path, function(subDirFiles) {
				...
				for (index in filenames) {
					checkFile(directory, filenames[index]);
				}
			}
		});
	}
	...
	var returnCrossDomain = function(){
		...
		net.createServer(function(socket)
		{
			...
			{
				if( data != '<policy-file-request/>\0' ) return;
				socket.write('\
<?xml version="1.0"?>\n\
...
<allow-access-from domain="*" to-ports="*" />\n\
	...
	if(utils.isWin()){
		...
		var realid = process.env.SUDO_USER;
		if(uid == 0) {
			returnCrossDomain();
			if (realid) {
				process.setuid(realid);
			}
		}
	...
	server.addListener('connection',function(stream)
		{
			utils.trace( "*** new connection ***" );
			stream.addListener ( 'close', function (haderror)
				{
					utils.trace( "*** stream close event *** ( with error = " + haderror + " )" );
				}
			);
		}
	);
	...
	server.addListener('close',function(errno)
		{
			utils.trace( "*** server close event = " + errno + " ***" );
		}
	);

	utils.info("Server running at http://localhost:" + gServerPort + "/" );
	var os = require('os');
	utils.info("Server os type : " + os.type());
	utils.info("current dir : " + process.cwd());
	...
	if(fileCount == 0) {
		...
		files.forEach(function (file, index) {
			fs.stat(path + '/' + file, function (err, statObj){
				...
				fileCount--;
				fileInfo.push({name:file, isFile: statObj.isFile(), mtime:statObj.mtime});
				if(statObj.isFile()) {
					filesToRead++;
				}
